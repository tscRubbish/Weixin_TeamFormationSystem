...
public class JwtUtil {
    ...
    // 秘钥
    static final String SECRET = "Nju-SE-Token";
    // 签名是有谁生成
    static final String ISSUSER = "NJU";
    // 签名的主题
    static final String SUBJECT = "this is nju se token";
    // 签名的观众
    static final String AUDIENCE = "WEB";

    public static String createToken(Integer userId, Integer userType, Integer expireMin) {
        try {
    ...
    public static Integer verifyTokenAndGetUserType(String token) {
        try {
            Algorithm algorithm = Algorithm.HMAC256(SECRET);
            JWTVerifier verifier = JWT.require(algorithm)
                    .withIssuer(ISSUSER)
            ...
            DecodedJWT jwt = verifier.verify(token);
            Map<String, Claim> claims = jwt.getClaims();
            Claim claim = claims.get("userType");
            return claim.asInt();
        } catch (JWTVerificationException exception){
            log.error(exception.getMessage());
        }
